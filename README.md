...
|--------|-------------|-------|--------|
| E1 | First-year expense | 800 | Beginning of year 1 |
| Er | Renewal expense (years 2 to n) | 80 | Beginning of each year |
...
A = sum_{t=0}^{n-1} v^(t+1) * d_(x+t)

where d_(x+t) = l_norm(x+t) * q_(x+t)

Note: A corresponds to A_30:20 (term life annuity factor).

### 3.3 Present Value of Premiums

a = sum_{t=0}^{n-1} v^t * l_norm(x+t)

Note: a corresponds to a_double_dot_30:20.
...
Equation:
P_gross * a = F * A + E1 + Er * (a - 1) + pi * P_gross * a

Solving for P_gross:

P_gross = [F * A + E1 + Er * (a - 1)] / [a * (1 - pi)]

### 3.6 Reserve Calculation (Recursive)

Starting with V0 = 0:

(V_t + P_net) * (1 + i) = q_(x+t) * F + (1 - q_(x+t)) * V_(t+1)

Solving for V_(t+1):

V_(t+1) = [(V_t + P_net) * (1 + i) - q_(x+t) * F] / (1 - q_(x+t))

where:
q_(x+t) = d_(x+t) / l_norm(x+t)
V_(t+1) >= 0 (reserve cannot be negative)

### 3.7 Annual Profit

Profit_t = P_gross - Expense_t - Claims_t - (V_(t+1) - V_t)
...
Let v_risk = 1 / (1 + r). Using mid-year discounting:

PV(Profit_t) = Profit_t * v_risk^(t + 0.5)

PVFP = sum_{t=0}^{n-1} PV(Profit_t)

### 3.9 Break-even Year

The smallest t such that cumulative PV(Profit) > 0.
...
**Background**: The original FAM mortality table was manually copied from fam_tables_2024.pdf (pages 20-21) into a text string inside parse_SULT.py. Python's pandas library then converted this text string into a DataFrame and exported it as an Excel file with the sheet name SULT.
...
- Normalizes lx (sets l_norm(30) = 1)
- Calculates A, a, P_net, P_gross
- Writes l_norm back to column D
...
| Metric | Value |
|--------|-------|
| Net premium (P_net) | $251.34 / year |
| Gross premium (P_gross) | $452.34 / year |
| PVFP | $101.74 |
